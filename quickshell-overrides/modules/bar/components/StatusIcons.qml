...
StyledRect {
    ...
    ColumnLayout {
        ...
        WrappedLoader {
            name: "network"
            active: Config.bar.status.showNetwork && (Nmcli.active || Nmcli.activeEthernet)

            sourceComponent: ColumnLayout {
                spacing: Appearance.spacing.smaller / 2
                property real meterSize: Config.bar.sizes.innerWidth - Appearance.padding.small * 2
                property real maxSpeed: 10 * 1024 * 1024

                Ref {
                    service: NetworkSpeed
                }

                SpeedMeter {
                    Layout.alignment: Qt.AlignHCenter
                    speed: NetworkSpeed.rxSpeed
                    icon: "download"
                ...
                SpeedMeter {
                    Layout.alignment: Qt.AlignHCenter
                    speed: NetworkSpeed.txSpeed
                    icon: "upload"
                    colour: Colours.palette.m3secondary
                }

                component SpeedMeter: Column {
                    required property real speed
                    required property string icon
                    required property color colour

                    property real normalizedSpeed: Math.min(1, speed / maxSpeed)
                    ...
                    StyledText {
                        anchors.horizontalCenter: parent.horizontalCenter
                        text: formatCompact(speed)
                        color: Colours.palette.m3onSurfaceVariant
                        font.pointSize: Appearance.font.size.smaller
                        font.family: Appearance.font.family.mono

                        function formatCompact(bytesPerSec) {
                            const kib = 1024;
                            const mib = kib * 1024;
                            if (bytesPerSec >= mib)
                                return (bytesPerSec / mib).toFixed(1) + "M";
                            if (bytesPerSec >= kib)
